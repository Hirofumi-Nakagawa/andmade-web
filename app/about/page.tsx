      ...
      <div className="hidden lg:contents">
        ...
        <div className="relative ml-[calc(198px*var(--grid-scale))] mt-[calc(270px*var(--scale))] w-[calc(var(--content-width)*var(--grid-scale))]">
          ...
          {/* 見出しからの 20px → 15px（いずれも直接の指示、"その下マージンを
              ...
              左のインデントは当初グリッドのマス数で持っていた（1マス58px、
              10マス=580px が添付デザインのリード文の左面 / その後 "1マス右に
              移動" で 11マス=638px）。今はマス数ではなく **日本語3行目の
              右端** を基準に決めている（直接の指示 "日本語3行目の
              「〜考えています。」の右端がグリッド右から3マス目右端で
              そろうようにして"）:

                目標の右端 = 24px + 58px × 22 = 1300px（右から3マス目の右端）
                3行目の実測幅 = 495.73px（1440px 時、18px / tracking 0.48px）
                左面 = 1300 - 495.73 = 804.27px
                この箱の左面（198px）を引いて pl = 約606px

              ＝左面はグリッド線には乗らない。文言・字送り・フォントを変えると
              3行目の幅が変わるので、そのときはこの606pxを測り直すこと。
              なお font-size は --scale、インデントは --grid-scale なので
              ぴったり揃うのは 1440px 時（両者が等倍になる幅）。 */}
          <RevealOnMount fadeOnly className="mt-[calc(15px*var(--scale))] pl-[calc(606px*var(--grid-scale))]">
