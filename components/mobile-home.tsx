...
export function MobileHome({ projects, news }: MobileHomeProps) {
  ...
  // instantly visible — per direct follow-up asking specifically for this,
  // matching site-header.tsx's own identical `fadeIn` prop/reasoning on PC
  // ("PC,SPのトップページが表示されるとき、ヘッダーもフェードインで表示す
  // るようにして。そしたらリロードした際にalpha0%のヘッダーはイントロが表
  // 示される瞬間に一瞬見える現象はなくなるはず"): starting genuinely
  ...
  // — per direct follow-up ("ヘッダーはイントロからトップに切り替わってから
  // フェードインするようにして"), matching site-header.tsx's own identical
  ...
  // per direct follow-up asking specifically to drop the fade for that case
  ...
  // (not just extra idle-hide triggers). returningToTop per explicit
  // follow-up ("back to topで上に戻ってる間は右下のイメージを出さないよう
  // にして"), matching PC's own suppressHoverPreview check in app/page.tsx.
  // footer-proximity-based suppression was removed entirely at one point
  // ("フッター表示でイメージを消す仕様は無し"), then reinstated per a
  // further direct follow-up ("一覧の一番下のイメージとタイトルがフッター
  // に干渉しそうになるタイミングでフェードアウトで消して"), then retimed to
  // a flat 30px lead-in ("...フッター要素の上マージン30pxくらいのところま
  // で来たら..."), then made exact — an actual geometric collision check
  // against SelectedProjectText's own bottom edge — per a still further
  // direct follow-up ("選択中に表示されるタイトル下のカテゴリが、フッター
  // の上面に触れたらイメージとタイトルとかがフェードアウトする仕様にし
  // て"), and finally also gated on the last project actually being active
  ...
  //
  // !showImages — per direct follow-up ("Th選択時はスクロールで背景イメー
        ...
        <div ref={sectionOriginRef} className="relative mt-[170px] pb-[20px]">
          <div className="absolute inset-0 flex items-start justify-between" style={{ top: RAIL_REST_OFFSET_PX }}>
            ...
            <div
              ...
              // railReleased (own fade-out once unstuck) — per direct
              // follow-up ("SPトップのtx-thと33 casesは固定解除されるタイミ
              // ングでフェードアウトで消える仕様にして"): only a fade was
              ...
              //
              // marginLeft: -4 — per direct follow-up ("左マージン4pxって
              // いうのは左のパーツ（tx-th、33）にだけ適用してほしい"): this
