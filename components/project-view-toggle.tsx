...
type ProjectViewToggleProps = {
   ...
   *  per direct request（貼付モック）: 以前は画面右下の CaseCounter が
...
/** カウントアップ（SlotDigits）を回し始めるまで — per direct follow-up
 *  （"casesのカウントアップはフェードイン後にスタート"）。レール自体が
 *  一拍おいて（LIST_ENTRANCE_DELAY_MS）500ms かけてフェードインするので、
 *  それが終わってから回し始める。 */
const COUNT_UP_START_MS = LIST_ENTRANCE_DELAY_MS + 500;

/** ブレンドモードは廃止し #000 に — per direct follow-up（"トップのヘッダー以外はブレンドモード無しにして文字色#000に"）。
 *
  ...
  return (
    ...
    <div className="absolute inset-0">
      <div
        ...
        data-name="btn"
        // 一覧と足並みを揃えて一拍おいてから出す — per direct follow-up
        // （"Made Hereとcases、お知らせなどもワンテンポ遅らせて表示"）。
        style={{ transitionDelay: `${LIST_ENTRANCE_DELAY_MS}ms` }}
      >
        {/* 「Made Here / N Cases」— per direct request（貼付モック）。
        ...
        <button
          type="button"
          // --underline-offset — 下線を既定（-0.1em）から1px下げる per direct
          // follow-up（"txt- imgの下線位置を1px下げて"）。bottom は負の
          ...
          // 選択中（current）は下線無し、非選択にだけ下線 — per direct
          // follow-up（"Txt - Imgも下線付けて（currentは下線無し）"）。
        ...
        {/* 区切りは "/" → "-" — per direct request（貼付モック "Txt - Img"）。 */}
